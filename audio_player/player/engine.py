...
def enumerate_alsa_hw_devices() -> list[dict]:
...
class AudioEngine(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._exclusive_device = "hw:0,0"

        self._eq_enabled = False
        self._eq_gains = [0.0] * 10

        self._emit_state_on_async_done = False
    ...
    # ------------------------------------------------------------------
    #  Bus message handler (called from Qt timer _poll — no GLib signals)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    #  Position / duration / bus polling (Qt timer, no GLib integration)
    # ------------------------------------------------------------------
    ...
    def _poll(self):
        ...
        if ok:
            ...
            # Stall detection: if PLAYING but position stuck at 0 (or same value)
            # for >3 seconds, the pipeline is likely deadlocked
            if self._app_state == PlaybackState.Playing:
                if ms == self._last_position_ms:
                    self._stall_ticks += 1
                    if self._stall_ticks > 60 and ms < 500:  # 3s at 50ms poll, stuck near start
    ...
    def audio_output(self):
        """Compatibility: returns output_info dict. Prefer output_info directly."""

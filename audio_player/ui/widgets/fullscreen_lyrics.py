...
class FullscreenLyricsWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Font cache: (size, bold, letter_spacing) -> (QFont, QFontMetrics)
        self._font_cache: dict[tuple, tuple[QFont, QFontMetrics]] = {}
    ...
    def _cached_font(self, size: int, bold: bool, letter_spacing: int) -> tuple[QFont, QFontMetrics]:
        key = (size, bold, letter_spacing)
        entry = self._font_cache.get(key)
        if entry is None:
            font = QFont()
    ...
    def paintEvent(self, event):
        ...
        if not self._lyrics:
            ...
            font, _ = self._cached_font(18, False, 0)
        ...
        for offset in range(-half_visible, half_visible + 1):
            ...
            font, fm = self._cached_font(int(fs), offset == 0, letter_spacing)
            painter.setFont(font)
            ...
            if translation and dist <= 2:
                ...
                t_font, t_fm = self._cached_font(t_fs, False, letter_spacing)
    ...
    def _paint_untimed(self, painter: QPainter):
        ...
        font, fm = self._cached_font(font_size, False, letter_spacing)
        t_font, t_fm = self._cached_font(max(10, font_size - 6), False, letter_spacing)
